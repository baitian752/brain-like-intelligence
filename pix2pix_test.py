...
parser.add_argument('--num_batchsteps', type=int, default=20)
...
class ImageFolder(data.Dataset):
    def __init__(self, opt):
        ...
        self.transform = transforms.Compose([transforms.ToTensor(),
                                             transforms.Normalize((0.5, 0.5, 0.5),
                                                                  (0.5, 0.5, 0.5))])
        self.dir_AB = os.path.join(opt.dataroot, 'test')  # ./maps/train
    ...
    def __getitem__(self, index):
        ...
        if ( self.no_flip) and random.random() < 0.5:
...
def to_variable(x):
    if torch.cuda.is_available():
        ...
        #x = torch.nn.DataParallel(model,device_ids=[1])
        x = x.cuda()
    return Variable(x)
...
def main():
    ...
    g_path = os.path.join(args.model_path, 'Generator-%d-%d.pkl' % (args.num_epochs,args.num_batchsteps))
    ...
    if torch.cuda.is_available():
        #torch.cuda.set_device(1)
        #generator = torch.nn.DataParallel(model,device_ids = [1])
    ...
    for i, sample in enumerate(data_loader):
        ...
        torchvision.utils.save_image(denorm(res.data),
                os.path.join(args.sample_path, 'Generated-%d.jpg' % (i + 1)))
